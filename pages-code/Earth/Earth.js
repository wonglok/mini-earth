  ...
  useEffect(() => {
    ...
    materialRef.current.onBeforeCompile = (node) => {
      ...
      node.fragmentShader = node.fragmentShader.replace(
        ...
        outgoingLight.r += hillColor.r * length((vHeight));
        outgoingLight.g += hillColor.g * length((vHeight));
        outgoingLight.b += hillColor.b * length((vHeight));

        // Seas
        if (length(vHeight) < length(vNormal)) {
          outgoingLight.r += riverColor.r * 0.003 / length((vHeight));
          outgoingLight.g += riverColor.g * 0.003 / length((vHeight));
          outgoingLight.b += riverColor.b * 0.003 / length((vHeight));
        }
        ...
        // hill top black
        outgoingLight.rgb -= length(vHeight) * 1.5;
